...
func (bot TipBot) createFaucet(ctx context.Context, text string, sender *tb.User) (*InlineFaucet, error) {
	...
	inlineMessage := fmt.Sprintf(Translate(ctx, "inlineFaucetMessage"), thirdparty.FormatSatsWithLKR(perUserAmount), GetUserStrMd(sender), thirdparty.FormatSatsWithLKR(amount), thirdparty.FormatSatsWithLKR(amount), 0, nTotal, MakeProgressbar(amount, amount))
	if len(memo) > 0 {
