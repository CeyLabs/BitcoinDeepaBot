...
func (bot *TipBot) sendHandler(ctx intercept.Context) (intercept.Context, error) {
	...
	confirmText := fmt.Sprintf(Translate(ctx, "confirmSendMessage"), str.MarkdownEscape(toUserStrMention), thirdparty.FormatSatsWithLKR(amount))
	if len(sendMemo) > 0 {
...
func (bot *TipBot) confirmSendHandler(ctx intercept.Context) (intercept.Context, error) {
	...
	// notify to user
	bot.trySendMessage(to.Telegram, fmt.Sprintf(i18n.Translate(to.Telegram.LanguageCode, "sendReceivedMessage"), fromUserStrMd, thirdparty.FormatSatsWithLKR(amount)))
	...
	if ctx.Callback().Message.Private() {
		...
		bot.tryDeleteMessage(ctx.Callback().Message)
		bot.trySendMessage(ctx.Callback().Sender, fmt.Sprintf(i18n.Translate(sendData.LanguageCode, "sendSentMessage"), thirdparty.FormatSatsWithLKR(amount), toUserStrMd))
	} else {
		// if the command was invoked in group chat
		bot.trySendMessage(ctx.Callback().Sender, fmt.Sprintf(i18n.Translate(from.Telegram.LanguageCode, "sendSentMessage"), thirdparty.FormatSatsWithLKR(amount), toUserStrMd))
		bot.tryEditMessage(ctx.Callback().Message, fmt.Sprintf(i18n.Translate(sendData.LanguageCode, "sendPublicSentMessage"), thirdparty.FormatSatsWithLKR(amount), fromUserStrMd, toUserStrMd), &tb.ReplyMarkup{})
	}
