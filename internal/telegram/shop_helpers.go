...
func (bot TipBot) shopItemConfirmBuyMenu(ctx intercept.Context, shop *Shop, item *ShopItem) *tb.ReplyMarkup {
	shopItemBuyButton = shopKeyboard.Data(fmt.Sprintf("💸 Pay %s sat", utils.FormatSats(item.Price)), "shop_itembuy", item.ID)
...
func (bot TipBot) shopMenu(ctx intercept.Context, shop *Shop, item *ShopItem) *tb.ReplyMarkup {
	...
	buyButtonText := "📩 Get"
	if item.Price > 0 {
		buyButtonText = fmt.Sprintf("Buy (%s sat)", utils.FormatSats(item.Price))
	}
